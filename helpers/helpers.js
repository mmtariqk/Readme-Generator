...
function ifAnsBasicsAdd(githubUsername, email, hireLink) {
    ...
    if (hasGithubUsername || hasEmail || hasHireLink) {
        ...
        if (hasGithubUsername)
            concatenate += "- Where can I see more of your repositories?\n\t- Visit [" + githubUsername + " Repositories](https://github.com/" + githubUsername + ")\n\n";

        if (hasEmail || hasHireLink) {
            concatenate += "- What is your email address?";
        }

        if (hasEmail) {
            concatenate += "\n\t- You can reach out if you have more questions at <a href='mailto:" + email + "'>" + email + "</a>.";
        }

        if (hasHireLink) {
            concatenate += "\n\t- If you are interested to [hire me](" + hireLink + ")?"
        }
...
function ifAnsAddText(answer, alternateConcatenation) {
    let concatenate = "";
...
function addRBadges(license, badges) {
    let concatenate = "";
    if (license && license.length && badges && badges.length) {
        concatenate = getLicBadge(license) + " " + badges + "\n\n";
    } else if (license && license.length) {
        concatenate = getLicBadge(license) + "\n\n";
...
module.exports = { ifAnsAddText, addTableOfContents, ifAnsBasicsAdd, getLicText, getLicBadge, addRBadges }